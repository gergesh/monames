...
checkWinner g
  | allRev Black = Just . otherTeam $ g ^. gameTurn . playerTeam
  | allRev Blue  = Just BlueTeam
  | allRev Red   = Just RedTeam
  | otherwise = Nothing
  where
    allRev col = areAllRevealed col (g ^. gameBoard)

-- | Set a game's State appropriately.
checkGameOver :: Game -> Game
checkGameOver g =
  case checkWinner g of
...
addNeededPlayer gp
  ...
  | otherwise = (countSmallerTeam +~ 1 $ gp, Player smallerTeam Guesser)
  where
...
checkNeedsNewClue g =
  ...
  where
    enemySpymaster = (playerRole .~ Spymaster) . (playerTeam %~ otherTeam)
